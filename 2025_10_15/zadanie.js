...
http.createServer(async (req, res) => {
    ...
    switch(urlObject.pathname){
        ...
        case '/htmlcode':
            ...
            break
        case '/htmlfile':
            const file = await fs.readFile('strona.html', 'utf8')
            res.end(file)
            break
        case '/get_params':
        ...
        default:
            try{
                await fs.stat(path.join(__dirname, 'assets', urlObject.pathname))
                fileType = mime.lookup(urlObject.pathname)
                res.setHeader('Content-Type', fileType)
                res.statusCode = 200
                let fileContent = await fs.readFile(path.join(__dirname, 'assets', urlObject.pathname), 'utf8')
                res.end(fileContent)
            }
            catch(error){
                res.statusCode = 404
                res.setHeader('Content-Type', 'application/json')
                let answer = {error: "Not found!"}
